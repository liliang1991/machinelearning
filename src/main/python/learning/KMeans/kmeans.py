    ...
    def fit(self,X):
        ...
        while (len(centList) < self.n_clusters):
            ...
            for i in range(len(centList)):#尝试划分每一族,选取使得误差最小的那个族进行划分
                ...
                #np.nonzero(self.clusterAssment[:,0]!=i)[0] 把clusterAssment 所有行第一列 不等于i 的全部输出
                #sum(self.clusterAssment[np.nonzero(self.clusterAssment[:,0]!=i)[0],1])  把clusterAssment 所有行第一列 不等于i 的平方误差输出
